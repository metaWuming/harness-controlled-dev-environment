...
| **gstack `/review`** | Step 5 第二道 review(Claude adversarial + Codex challenge 組合) | Claude Code 內建 `/code-review` + 派**一個** `.claude/agents/adversarial-reviewer.md` 對 diff 獨立審(**從 diff 出發,不是去驗證前一輪的結論**——那會複製盲點)。高風險車道另加的 worktree 獨立審**與本工具在不在無關**,做法只寫在 SOP Step 5〔僅高風險車道〕條目(單一正本,此處不重抄) | 組合式多 pass 的覆蓋面 |
