...
| CTRL-CI-013 | Mutation spec 樣本漂移守門 | ⑪ | `push` `pull_request` | `.github/workflows/ci.yml`, `scripts/check-mutation-specs.ts`, `scripts/lib/invoked-as-main.ts` | CI step「Mutation Spec Drift Check」;對 scripts/mutations/*.json 每條探針驗 find 樣本仍能在目標原始碼精準對上(不跑 mutation);spec 檔與目標檔都先經 mutate.ts checkTarget(repo 內、tracked、非 symlink)取 bytes | `Mutation Spec Drift Check` | github | block | admin override | CI check 狀態;本機 npm run check:mutation-specs 印每個 spec 檔的探針數,exit 1 = DRIFT(內容層、改 spec 或碼)/ exit 2 = 無法判定(spec 目錄或檔案不可信、0 個 spec 檔) | spec 目錄非真目錄 / 0 個 spec 檔 / spec 檔本身 symlink、未追蹤、非一般檔、hardlink、非 UTF-8 → exit 2(無法判定,不放行) | unit e2e mutation | `tests/check-mutation-specs.test.ts`, `scripts/mutations/mutation-spec-drift.json` | 只驗「樣本對得上」,不證明探針仍會 kill;完整 mutation 仍由 Step 4.5 人工跑並綁 HEAD。是 mutate.ts 閘② 的 CI 前置版 |
