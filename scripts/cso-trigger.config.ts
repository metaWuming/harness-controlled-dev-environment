...
//
// ⚠️ 模板出廠狀態:**路徑表為空**。空表下 `scripts/check-cso-trigger.ts` 永遠輸出
//   CSO_NOT_REQUIRED —— 這代表「尚未導入」,不代表「你的 repo 沒有安全敏感面」。
//   導入時把你的 repo 內對應五個域的路徑填進來(每域下方有註解掉的通用範例),
//   然後啟用 tests/check-cso-trigger.test.ts 內註解掉的「路徑表完整性鎖」測試。
//
