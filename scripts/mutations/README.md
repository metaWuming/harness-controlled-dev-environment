...
- exit **2** = 無法判定:`scripts/mutations` 不是真目錄、0 個 spec 檔、spec 檔本身是 symlink / 未追蹤 / 非一般檔 / 非 UTF-8 / root 解析失敗(fs.realpathSync throw)/ argv 錯(parseRootArg fail-closed)。**未預期例外**:main try/catch 印訊息但不重設 exit code(保留當前 code:runCheck 前拋 = initial 2、runCheck 已回報後拋 = 該次 report code 0/1)
