...
//     Step 5 寫 progress entry 時同步貼進 `.claude/memory/progress.md` 留史 →
//     Step 6 開 PR 時也貼進 PR 描述(合併前的關口)——兩處貼、不轉抄**。
//     ⚠️ 舊寫法「Step 7 再抄進 progress」已作廢(2026-08-21):Step 7 是 post-merge、
//     develop protected 動不了,為抄一行 progress 另開 PR = 每 sprint 收尾多 1 支 PR
//     + 1 輪 CI 純浪費。改成 Step 5 一起寫、跟 code 同一 squash。
...
function main(): void {
  ...
  console.log(
    `🔎 量詞自檢:${lines.length} 行新增內容,**${hits.length} 處**要逐條過目(base=${base})\n` +
      ...
      '   列得出來 → 留著;列不出來 → 降級措辭。\n' +
      '   🔴 逐條處置要在**合併前**貼進 PR 描述,並在 Step 5 一起寫進 progress.md 留史(兩處貼、不轉抄)。\n',
  );
