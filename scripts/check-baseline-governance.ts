...
//      `--head` ∈ **merge-base 那側** harness.config 宣告的 protectedBranches
...
export interface GovernanceOptions {
  /**
   * PR head 分支名(同 repo PR 才傳)。∈ **merge-base 那一側**的 harness.config `protectedBranches` → SKIPPED。
...
export function evaluateBaselineGovernance(baseRef: string, io: GitIo, opts: GovernanceOptions = {}): GovernanceResult {
  ...
  if (opts.headRef !== undefined && opts.headRef !== null) {
    ...
    if (mbCfgText !== null) {
      try {
        ...
        infoLines.push(`  [info] merge-base 的 ${HARNESS_CONFIG_PATH} 解析失敗(${(e as Error).message.slice(0, 80)}),不套用「head ∈ protectedBranches」豁免`);
      }
    } else {
      infoLines.push(`  [info] merge-base 沒有 ${HARNESS_CONFIG_PATH},不套用「head ∈ protectedBranches」豁免`);
    }
    if (protectedAtMb !== null && protectedAtMb.includes(opts.headRef)) {
      return {
        ...
        findings: [],
        lines: [
          `BASELINE_GOVERNANCE_SKIPPED — head ${opts.headRef} ∈ merge-base 的 protectedBranches,其內容進入 ${opts.headRef} 時已逐 PR 受本 gate 檢查(fork PR 不會走到這裡;政策讀自 merge-base,PR 改不到)`,
  ...
  //   不套用『head ∈ protectedBranches』豁免」)會在讀者眼前消失、影響診斷判讀。
...
// ⚠️ 本 script static-import check-bookkeeping-commit + check-no-source-terms,
//    e2e case #4(indeterminate)必須用 3-step wrapper(先 cache 這兩支再改 argv1),
//    見 tests/fixtures/invoked-as-main-wrapper/check-baseline-governance-wrapper.mjs。
const outcome = detectInvocation(import.meta.url, process.argv[1]);
const isMain = reportIfNotMain(outcome, 'check-baseline-governance');
if (isMain) {
  process.exit(main());
