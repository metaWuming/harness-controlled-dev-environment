 ...
 *
 * --root 契約(P2#3 defer ⑪):`<dir>` 必須是 **real git worktree top-level**。
 *   若指向外層 git repo 的子目錄且該子目錄被外層 tracked → tracked 判定會用外層 repo
 *   git index(信任邊界依賴外層 repo、非 --root 認可的 boundary),**不支援、屬使用者
