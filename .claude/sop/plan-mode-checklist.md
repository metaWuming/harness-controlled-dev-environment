...
- [ ] 🔴 **每個 finding 再標一個二分 tag:「行為級」或「散文級」——散文級不消耗確認輪。**
      判準看的是「finding 的**完整修法要動什麼**」,不是「這段文字有沒有被程式執行」:
...
- [ ] **修完一輪、送下一輪之前,把「自己這輪新加的東西」當成一份新 diff 自檢一遍**
      ...
      等於花一整輪請對手模型幫你審第一遍。**這一步要加硬成三句可執行的**
      (「自檢一遍」這種說法不可執行,實測連幾刀都沒擋住):
  - **修法引入新機制**(新函式／新 timer／新守衛／新狀態)→ 送下一輪前對它跑**至少一條 mutation 探針**
    (手動改壞 → 看某條測試轉紅 → 還原)。工作樹髒時用手動探針即可,但**要明講那不是完整的 mutation 掃描**。
  - **修法改了任何時序常數、或把某個動作往後排** → 先回答「**哪一條既有測試的 tick 現在推不到它了?**」
    真實案例:把破壞性動作從 15 秒延到 75 秒、而那條測試只推進到 30 秒 →
    **同一條 mutation 從被抓變成存活,測試一個字都沒改**。
  - **修法寫了新的宣稱句** → 對**這一輪的 diff** 跑 `npm run check:claims -- --base=<上一輪送審的 HEAD>`
    (量詞自檢器:掃本輪 diff 新增行裡的量詞與絕對化措辭、產待處置清單)。
    🔴 **一定要帶 `--base`**:預設 base 是主線,round 2 之後每次都會把整支 PR 重掃一遍,
    **前幾輪早處置完的幾十個命中會把本輪新增那幾句淹掉**——正好抵銷這條規則的用意。
    (未提交的修法照樣被 `git diff <base>` 收進去,不必先 commit。)
    🔴 **那個 SHA 要有地方拿**:**每次送審的 prompt 裡都寫上 `git rev-parse HEAD` 的值**;
    下一輪的 prompt 同時列「上一輪 reviewed HEAD」與「本輪 HEAD」,`check:claims` 用前者當 `--base`。
- [ ] **finding 涉及「一個被宣稱的不變量」時,連它的守法一起審,不只審那句敘述**:
  - **敘述只准一份 SSOT**:一個不變量的完整敘述放**單一錨點**,其他入口只放指標(`[見 SSOT: <slug>]`)、
    不放摘要。**指標與摘要並存＝指標形同虛設**(兩份會漂移,而漏改的 consumer 往往就是同一條敘述散在 4–6 處)。
  - **呼叫點要另外守**:純函式有單元測試也有 mutation ≠ 保護在生效——把呼叫它的那一行刪掉,兩者照樣全綠。
    判準:問「**我刪掉這行呼叫,有什麼會紅?**」修法是把接線那段也抽成可注入的純函式,
    用拋棄式環境(暫存目錄＋真實資源)或注入的 I/O 替身驗它。
  - **守門優先用「位置＋數量」,別拿字串處理去解析有結構的東西**(每補一次就換一個洞);
    **白名單用「位置」不用「值」**(以值為準的豁免必然全域,第二個同值的違規就溜過去);
    **要鎖就鎖完整集合、連值一起鎖**——能被「刪一條真的、補一條假的」湊過的正對照等於沒有。
  - ⚠️ 這種「位置＋數量型」守門守得比你以為的窄:抓得到錨點消失／重複／搬檔／野指標／死錨點,
    **抓不到「沒帶標記的新摘要」**(那要比對自然語言,通常刻意不做)。知道邊界,別假設它擋得住語意漂移。
