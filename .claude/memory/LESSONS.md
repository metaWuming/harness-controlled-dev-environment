...
> ⚠️ **版本界線(僅限交付證據來源)**:上一項「workflow yml 修法」所描述的「三處同步」(包含傳入 `DELIVERY_REFS`)與「script 本身用四條來源」皆記錄批 6 當時實作;P2#2 移除 sprint(pull request 編號 49)後,`check:todos` 與 `check:no-source-terms` 的交付證據只取受驗的 `origin/HEAD`,不讀 `DELIVERY_REFS`,也不會 fallback 到固定的 `origin/develop` 或任何本地分支。現行實作契約見 `scripts/lib/delivery-refs.ts` 的模組說明與 `resolveDeliveryRefs`;若要更換交付線,依 `docs/MIGRATION.md` 附錄 A.1 同步檢查相關設定與 workflow,不可只改 `deliveryBranches`。
