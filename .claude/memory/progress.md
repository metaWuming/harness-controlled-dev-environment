...
📅 2026-08-27 ③ — **Harness backsync 批 6:LESSONS 記 self-PR gate 踩坑 + CI workflow delivery-branch 一致性契約**

> **緣起**:上一 sprint(#30,批 5)Step 6 連續踩到 3 個「self-PR # citation 撞去識別化 denylist」變體(test fixture / TODOS 補號 / CI push event 缺 MARKER_SELF_PR),Owner 拍板一批打包收:LESSONS 記那 3 條 + 修 CI workflow 讓 self-PR citation flow 順。起手 git 核實:main 已同步 #30(`40cac95`)、無 partial entry、乾淨工作樹。
> **改動**:3 檔——`.claude/memory/LESSONS.md`(新一條完整教訓、涵蓋 5 個「三處/四處同步、glob 不支援、round 6-like 風險」關鍵細節);`.github/workflows/ci.yml`(TODOS Markers Check + fetch step 條件從「pull_request event only」逐輪收乾成 delivery-branch 白名單:PR event 一律跑 / push 只在 default_branch 或 develop 進;event filter 涵蓋 main/master/trunk/develop;fetch 用 default_branch 動態抓 + set-head + DELIVERY_REFS env belt-and-suspenders);`scripts/check-todos-markers.ts`(refs 建立邏輯改「①origin/HEAD → ②env DELIVERY_REFS → ③fallback origin/develop → ④last-resort 本地」四條合流,`origin/develop` 挪 fallback 避免 legacy 分支假通過;`execFileSync + arg array + SAFE_REF_RE whitelist` 擋 shell 注入與 `--all` option injection)。
> **審查**:Step 4 Codex CLI review rounds 1-7(round 1-6 各一條 P2 行為級——所有本以為簡單的 `if:` 條件被逐輪拆出「event filter 上層過濾」「default_branch 硬碼」「DELIVERY_REF_CANDIDATES 沒同步」「擴充候選反成 legacy 假通過」等新面向;round 7 未發現新缺陷收乾)。安全關:`check:cso` fail-closed(表空)→ 模板 repo 例外(路徑表空為設計)+ 人工自問——本 sprint 動 CI workflow + 守門 script,**屬安全敏感面**(script 決定 marker 是否算 merge 證據、workflow 決定 gate 是否跑);但**沒新增 auth/金流/個資邏輯**(只改守門判定順序 + defense-in-depth 擋注入)→ 屬**強化既有安全面、非新增安全面**,不進高風險車道(採用相對寬鬆判定,若下 sprint 累積類似案例,再檢討是否要把「守門 script 邏輯改動」也視同 REQUIRED)。視覺關:未觸發(無 UI)。Step 5:adversarial-reviewer fresh 審——0 CRITICAL / 8 INFORMATIONAL,confidence 5-9 x7 修(#1 docstring 引用刪除的舊常數名、#2 origin/develop 無條件信任 = round 6 flaw 換位置重演、#3 LESSONS 說 glob 支援但 script 只字面比較、#4 shell metacharacter/option injection 防護、#5 docstring 說「三條」實作四條、#6 set-head 失敗 fail-hard、#7 warning 例子改直覺),confidence 4 x1 skip(#8 YAML 單引號 default_branch 邊角)。cross-model agreement ≈ 0(Codex 7 輪都在「delivery branch 定義的一致性」軸、Step 5 找到 codex 沒觸的 shell injection / doc drift / origin/develop 換位重演——meta 教訓再次驗證)。
> **驗證**:typecheck / lint 綠;tests 15 檔 388 passed(baseline 349,本 sprint 未加測試——CI workflow yml 邏輯與 marker script env 分岐尚未有 test harness);check:doc-refs 140 引用 0 失效;check:todos 綠(#30 引用 1 個驗到 merge 證據);check:no-source-terms 三段全綠。
> **⭐ 教訓**:①**「加一個 if condition」表面小改動、實際展開成 delivery-branch 定義的完整一致性契約**——round 1-6 各拆一個原本沒想到的邊角(event filter → default_branch → legacy branch → 沒有 glob → 換位重演),SOP 壓輪數紀律 ⑵「新機制自檢一遍」在**跨系統契約**這種形狀上實測值 = 每輪必找到新面向,直到 round 7 才收乾。教訓:改跨系統契約時、寫 fix 前就要問「三處/四處都涵蓋了嗎?」而非 review 逐輪拆。②**Codex 與 Step 5 看的軸完全不重疊**——Codex 7 輪全在「delivery branch 一致性」、Step 5 首次抓到 `execFileSync` shell 注入 + origin/develop 換位重演 + doc drift × 3——「cross-model agreement ≠ correctness」meta 教訓再次驗證。
> **⏭️ 下一棒候選**(hint 非 truth,起手 git 核實):A. `check-todos-markers.test.ts` 補 CI workflow / env 分岐測試(現在只覆蓋純函式 checkTodosMarkers、沒覆蓋 buildMergedPrSet 分支);B. workflow yml 邏輯本身沒有 test harness——考慮加 script 端 e2e(mock env、模擬 CI 環境變數組合);C. `check-no-source-terms.sh` denylist `PR #[0-9]` 從 regex denylist 升成「上下文感知 checker」(把 TODOS.md / progress.md 加豁免的 縮減 pattern 集)——教訓階梯升級預備(第 4 次同類 denylist 誤觸就該做)。
> **check:claims 逐條處置**:命中 0 處(本 sprint 全在 CI / script 邏輯層,新增行未觸發量詞 pattern)。
> 📊 成本:CC ~2h / 跨模型 review 7 rounds + Step5 1 / P1 0 個 / P2 8 個(round 1-6 各 1 + round 4 額外 1)/ Step5 獨立發現 8 個(修 7、skip 1)
> 📐 量測(供 `docs/EFFORT.md` sweep;人工填):每輪 model＋effort:主迴圈 claude-opus-4-7(session 預設 effort);Codex exec r1-6 non-interactive review 預設 / baseline SHA:`7ed0d7d2cbe0b95ec8c03020c979cb40a44d24a8`(Phase 2 完成、review round 1 前的 HEAD)/ 來源分佈:初始 patch 內既有缺陷 2(Phase 2 初版把 push event 完全 skip、CI push event 沒傳 MARKER_SELF_PR 屬 pre-existing 但本 sprint 是首次觸及)・初始 patch 漏改的外部 consumer 3(DELIVERY_REF_CANDIDATES / workflow fetch step / event filter 三處沒同步)・baseline 後新增/修改引入 3(round 5 擴充候選反成 round 6 flaw / round 4 動態 default_branch 反暴露 event filter 限制 / Step 5 origin/develop 挪位是這批的自我修正)
> **7 步 checklist 狀態**:Step 1 ✅(單一 sprint 短 plan、無真實取捨)/ Step 2 ✅(全 sensible default 通過)/ Step 3 ✅(2 phase atomic commits、gate 綠)/ Step 4 ✅(7 rounds 收乾)/ 4.5 ✅(人工判定「強化既有守門」不進高風險車道)/ 4.6 ✅(未觸發)/ Step 5 ✅(含本 entry bookkeeping)/ Step 6-7 待執行
