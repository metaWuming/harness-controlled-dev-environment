...
> **驗證**:先寫契約、對**舊實作**跑一次記錄基線 —— 8 條 scale 契約紅(C1/C2a/C2b/C2d/C3/C4/C5p/C6)、15 條行為契約綠;實作後全部轉綠。最終:16 檔 **557 tests** / typecheck / lint / check:doc-refs 256 引用 0 失效 / **check:doc-size progress 通過**(本 entry 收尾又 archive 了 PR A1 一條) / check:hooks / check:todos / check:bookkeeping / check:no-source-terms 主線綠(0.38s)。
> **mutation 探針**:**17 條 mutations 全數被抓**(13 條既有 + R1 輪新增 M12/M13/M14/M15),`npm run mutate` exit 0;探針在本 sprint 最後一個 commit 的樹狀態上執行,完整綁定 SHA 見交接報告。三次存活各自暴露不同的覆蓋缺口:
...
> 📊 成本:CC ~7h / plan 階段 supervisor review 3 輪 + 實作後 Codex round 1 / P1 1 個(R1,已修)/ P2 5 個(F1・F2・F3 + R1 兩條,全修)/ Step5 獨立發現 4 個(自審批次體積退化、兩次 mutation 存活暴露的覆蓋缺口、修 P1 時自查抓到的 grep -z 冒號截斷假放行,全修)。
...
> **7 步 checklist 狀態**:1 ✅(plan 在 repo 外、含 impact radius / 正反契約 / mutation / migration / rollback / D1-D11)/ 2 ✅(supervisor R1-R2 修訂後 R3 批准;Owner-only decision = none)/ 3 ✅(tests-first / implementation / docs / self-review / bookkeeping commits;最終 gate 全綠)/ 4 ⏳ Codex round 1:1 P1 + 2 P2;Claude 已修並提交,等待原 baseline..最新 tip 全範圍複審 / 4.5 ✅(高風險車道:16 條探針 exit 0 綁 SHA)/ 4.6 ✅(not-applicable)/ 5 ⏳ Codex commit-only independent review 尚未完成;Claude self-review 不取代獨立 gate / 6-7 待執行
