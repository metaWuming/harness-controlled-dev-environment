...
> **check:claims 逐條處置**:31 命中(base=origin/main、cf2feaa):25 條 pre-existing 在 base cf2feaa(Sprint 4 已 merge 內容,分佈 progress-archive 9、progress.md 5、TODOS.md 4、MIGRATION.md 4、其他 3)、6 條由本 sprint 引入——(a) tests/check-no-source-terms.test.ts:414 / :432 / :448「never」(production 等價 argv `--color=never` 逐字引用、technical arg 非絕對化宣稱)、(b) tests/check-no-source-terms.test.ts:420 / :422「只有」(comment 描述 F1a / F1b guard「只有 line 或 column 非空純數字才視 3-NUL」、精確 scope 描述)、(c) TODOS.md L136「保證」(交付段「兩半保證都機器化鎖住」、guard contract 描述)——**全部 KEEP**:new hits 皆為 technical arg 逐字引用或 guard 契約精確描述、無替代 wording;pre-existing 25 條屬 Sprint 4 已 merge 內容、非本 sprint 引入。0 個 actionable 新絕對化宣稱句。
