...
| **4. Remote & long-term** (after pushing) | ⑪ Always-on CI gates (least-privilege `GITHUB_TOKEN`, SHA-pinned actions, full-history gitleaks with pinned binary + sha256, dependency audit hard-gate on high/critical, doc-reference integrity, TODOS marker governance, full test suite, and an **adoption-readiness gate** — `check:adoption` reads the declared `mode` in `scripts/harness.config.json` and, for an adopted project, fails closed on every leftover placeholder, empty sensitive-path domain, skeleton section or branch-policy mismatch; the template itself lists its exceptions explicitly instead of pretending to be complete) ⑫ Weekly health check — three collectors: work backlog, lesson-production rate, and **review-dulling detection** (review rounds and P1 counts per sprint, parsed from progress cost fields) ⑬ Quarterly retro (archive mechanized lessons, keep the memory lean, scan doc cross-references for drift) |
...
- **Main-only alignment** — shipped `protectedBranches` in `scripts/harness.config.json` corrected from `["develop", "main"]` to `["main"]` (live probe shows no remote `develop` ref; avoids stale claim).
...
- **Dogfooding** — this template eats its own gates: its CI runs gitleaks over full
  ...
  work-management gates run through to completion
...
| **四、遠端與長期層**(推出去之後) | ⑪ CI 常駐閘門(`GITHUB_TOKEN` 最小權限、actions SHA-pin、gitleaks 全史掃描〔pinned binary+sha256 校驗〕、依賴稽核 high/critical 硬擋、文件引用完整性、TODOS marker 治理、全套測試,以及**導入就緒閘門**——`check:adoption` 讀 `scripts/harness.config.json` 宣告的 `mode`,adopted 專案有任何殘留 placeholder、空的敏感域、骨架段落或分支政策不一致就 fail-closed;模板自身則逐條列出 exception、不假裝完成) ⑫ 週健檢——三個 collector:工作累積、教訓產出速率、**審查鈍化偵測**(每 sprint 的 review 輪數與 P1 數,從 progress cost field 解析) ⑬ 季 retro(封存已機器化教訓、記憶防膨脹、掃文件交叉引用防漂移) |
...
- **Main-only 對齊**:shipped `scripts/harness.config.json` `protectedBranches` 由 `["develop", "main"]` 校正為 `["main"]`(live probe 顯示目前沒有 remote develop ref、避 stale claim)。
