...
- `npm run check:baseline-governance`(`scripts/check-baseline-governance.ts`)+ CI step「Baseline Governance Check」(pull_request only):source-term baseline 只能在「只動 config + ADR + bookkeeping」的 PR 內往前推到 merge-base 祖先;base 用 PR 的 base 分支;同 repo 的保護分支之間 promotion PR(依 merge-base 那側宣告的 `protectedBranches`)明文 SKIPPED。
